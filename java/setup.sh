...
download_file() {
  if [ $setup_type != 'docker' ] && [ -n "$download_url" ] ; then
    ...
    yum install -y wget
    rm -f $build_dir/*.gz
    wget -c --no-cookies --no-check-certificate --header "Cookie:oraclelicense=accept-securebackup-cookie" $download_url -P $build_dir
  fi
}
...
set_env() {
  if [ $setup_type != 'docker' ] ; then
    echo '设置环境变量...'
    echo >> /etc/profile
    echo "export JAVA_HOME=$setup_dir" >> /etc/profile
    echo "export JRE_HOME=$setup_dir/jre" >> /etc/profile
